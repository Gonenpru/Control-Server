...
public class ArrivalManager {
	...
	public boolean checkAirportSpace(Planes plane) {
		...
		try {
			...
			Launcher.PLANE_MOVEMENTS.put(plane.getId(), new PlaneMovements(id, plane.getId(), Arrival.AIRPORT.getX(), Arrival.AIRPORT.getY()));
			Transaction trans = Launcher.session.beginTransaction();
			Launcher.session.save(Launcher.PLANE_MOVEMENTS.get(plane.getId()));
			trans.commit();
